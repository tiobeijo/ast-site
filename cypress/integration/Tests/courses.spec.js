describe('coordenador role test', function() {
  ...
  .then(win => {
   ...
   it('Checking information', function() {
    cy.contains('input[name="files[field_imagem_de_destaque_0][]"]').click()
    cy.contains('input[name="field_requisitos[0][value]"]').type('Idade Mínima: 16 anos')
    cy.contains('input[name="field_requisitos_add_more"]').click()
    cy.contains('input[name="field_requisitos[1][value]"]').type('Morar em Belo Horizonte')
    cy.contains('input[name="field_requisitos_add_more"]').click()
    cy.contains('input[name="field_requisitos[2][value]"]').type('Escolaridade Mínima: 1° Ano/Ensino Médio')
    cy.contains('input[name="field_requisitos_add_more"]').click()
    cy.contains('input[name="field_requisitos[3][value]"]').type('Possuir Cadastro Único')
    cy.contains('Salvar').click()
   }) 
})